...
public class VisionCone : MonoBehaviour
{
    ...
    void DrawVisionCone()
    {
        ...
        for (int i = 0; i < VisionConeResolution; i++)
        {
            ...
            // Raycast to check if there's an obstacle in the direction
            RaycastHit hit;
            if (Physics.Raycast(transform.position, direction, out hit, VisionRange, VisionObstructingLayer))
            {
                // If we hit an obstacle, set the vertex at the hit point
                vertexPosition = transform.InverseTransformPoint(hit.point);
            }
            else
            {
                // Otherwise, set it at the maximum range
        ...
        // Position the vision cone slightly above the enemy
        transform.localPosition = new Vector3(0, 0.5f, 0); // Adjust Y value as needed
    }
